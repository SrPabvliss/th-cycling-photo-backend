...
import { SetEventPricingConfigHandler } from './set-event-pricing-config.handler'

describe('SetEventPricingConfigHandler', () => {
  let handler: SetEventPricingConfigHandler
  ...
  const command = new SetEventPricingConfigCommand(
    EVENT_ID,
    { currency: 'USD', tiers: [{ minQty: 1, maxQty: null, pricePerPhoto: 5 }] },
    'caller-1',
  )
  ...
  it('throws NOT_FOUND — not FORBIDDEN — when the event exists but is outside the caller scope', async () => {
    // Ruling 21. `pricing.config.set` is not platformOnly, so it is grantable
    // to a tenant even though no template holds it today; without the scoped
    // load a grant-holder could rewrite any event's pricing by UUID.
  ...
  it('rejects an invalid tier before writing anything', async () => {
    const invalid = new SetEventPricingConfigCommand(
      EVENT_ID,
      { currency: 'USD', tiers: [{ minQty: 0, maxQty: null, pricePerPhoto: 5 }] },
      'caller-1',
    )
  ...
  it('persists the config once the scoped event is found and permission holds', async () => {
    await handler.execute(command)

    expect(authz.assert).toHaveBeenCalledWith('caller-1', 'pricing.config.set', EVENT_ID)
    expect(repo.upsertConfig).toHaveBeenCalledWith(EVENT_ID, {
      currency: 'USD',
