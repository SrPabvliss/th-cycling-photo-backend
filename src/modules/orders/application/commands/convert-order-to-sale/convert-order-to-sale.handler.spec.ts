...
describe('ConvertOrderToSaleHandler', () => {
  ...
  function buildHandler() {
    ...
    const handler = new ConvertOrderToSaleHandler(writeRepo as never, readRepo as never, authz)
  ...
  it('throws NOT_FOUND — not FORBIDDEN — when the order exists but its event is outside the caller scope', async () => {
    ...
    const error = await handler
      .execute(new ConvertOrderToSaleCommand('other-tenant-order', audit))
      .catch((e) => e)
  ...
  it('converts the order and saves it', async () => {
    ...
    expect(authz.assert).toHaveBeenCalledWith('admin-1', 'order.convert_to_sale', 'event-1')
