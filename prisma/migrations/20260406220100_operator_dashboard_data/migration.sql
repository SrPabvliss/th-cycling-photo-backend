-- Migrate classifier rows to operator (uses the enum value committed in
-- previous migration) and replace role_type with a clean enum without
-- 'classifier'. Then add event_operators table and retouched_by_id on
-- photos.
UPDATE "roles" SET "name" = 'operator' WHERE "name" = 'classifier';

-- Swap to clean enum without 'classifier'
